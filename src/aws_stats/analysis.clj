...
(defn show-number
  [url]
  (let [[_ show-number] (re-matches #".*-(\d\d\d)-.*" url)]
    (or show-number "")))

(defn show-downloads-by-day-csv
  "Returns a map of S3 URIs to a map of dates to the equivalent number
  of times it has been downloaded. A download equivalent is the number
  of bytes transferred divided by the object size."
  [db]
  (->> (d/q '[:find ?uri ?date (sum ?download)
              ;; We need a :with because we don't want to sum
              ;; *unique* downloads, but *all* downloads
              :with ?request-id
              :where
              ...
              [(/ ?bytes-double ?object-size) ?download]]
            db)
       (filter (fn [[uri]] (.contains uri ".mp3")))
       (map (fn [[uri date downloads]]
              [uri (show-number uri) date downloads]))
       (map #(str/join "," %))
       (str/join "\n")))
